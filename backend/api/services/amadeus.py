...
class AmadeusService:
    ...
    def _initialize_client(self) -> None:
        ...
        try:
            ...
            env_name = (
                "teste" if settings.amadeus_env.lower() != "production" else "produção"
            )
        ...
        for attempt in range(max_retry):
            try:
                ...
                if not response.data:
                    logger.info(
                        f"Nenhuma oferta encontrada para {origin}-{dest} em {day}"
                    )
                    return None

                # Encontrar o menor preço
                prices = [
                    float(offer["price"]["grandTotal"]) for offer in response.data
                ]
                cheapest = min(prices)
